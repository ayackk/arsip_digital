...
class ArsipForm
{
    public static function configure(Schema $schema): Schema
    {
        return $schema->schema([
            TextInput::make('judul_arsip')->required()->maxLength(255),
            DatePicker::make('tanggal_naskah')->required(),

            Select::make('unit_pengolah_id')
                ->relationship('unitPengolah', 'nama_unit')
                ->default(Auth::user() ? Auth::user()->unit_pengolah_id : null) // Otomatis terisi Unit si user
            ...
            Hidden::make('created_by')->default(Auth::user() ? Auth::user()->id : null),
        ]);
    }
}
